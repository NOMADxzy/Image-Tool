...
class ImageGallery extends React.Component {
    ...
    onThumbnailClick(event, index) {//点击
        ...
        switch (key) {
            ...
            case ESC_KEY:
                if (isFullscreen && !useBrowserFullscreen) {
                    this.exitFullScreen();
                }else this.props.setselectedImg(false)
                break;
            default:
                break;
        }
    ...
    handleDetect(items){
        ...
        //     items[this.state.currentIndex].original = items[this.state.currentIndex].thumbnail
        // }
        this.setState({Detected:true})
        fetch('http://localhost:5000/detect/', {
            method: 'POST', // or 'PUT'
            body: JSON.stringify({source:items[this.state.currentIndex].id}), // data can be `string` or {object}!
            headers: new Headers({
    ...
    render() {
        ...
        const slideWrapper = (
            <div ref={this.imageGallerySlideWrapper} className={slideWrapperClass}>
                ...
                {renderDetectButton(this.handleDetect.bind(this,items),Detected)}
                {
                    showBullets && (
...
ImageGallery.defaultProps = {
    ...
    renderDetectButton: (onClick, Detected) => (
        <Detect onClick={onClick} Detected={Detected} />
    ),
